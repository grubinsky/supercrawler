...
Crawler = function (opts) {
  ...
  this._checkRobotsTxt = opts.checkRobotsTxt !== "undefined" ? opts.checkRobotsTxt : true;
};
...
Crawler.prototype._processUrl = function (url) {
  ...
  var download = this._checkRobotsTxt ?
                   this._downloadAndCheckRobots(url).then(function () { return self._downloadUrl(url, false) }) :
                   self._downloadUrl(url, false);

  return download.then(function (_response) {
    var contentType,
        statusCode,
        location;
